...
function WordDetailCard({ card, onSave, onBack, form, prefLang }) {
  ...
  return (
    <div style={{ position:"relative" }}>
      ...
      {/* ── IMAGE ── */}
      <div style={{ ...S.card, marginBottom:12 }}>
        <p style={{ color:"#64748b", fontSize:11, fontWeight:700, letterSpacing:1, margin:"0 0 10px" }}>🖼 IMAGE ASSOCIATION</p>
        {!imgError ? (
          <img
          ...
          <div style={{ width:"100%", height:130, borderRadius:10, background:"rgba(255,255,255,0.04)", display:"flex", alignItems:"center", justifyContent:"center", border:`1px dashed ${C.border}` }}>
            <span style={{ color:"#475569", fontSize:13 }}>🌸 {card.word}</span>
          </div>
        )}
          ...
          {words.map((w,i) => (
            <div key={i} style={{ ...S.card, borderLeft:`3px solid ${C.teal}`, cursor:"pointer" }} onClick={()=>{setSelectedWord(w);setVocabView("wordDetail");}}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                <div style={{ flex:1 }}>
                  <div style={{ display:"flex", alignItems:"baseline", gap:8, marginBottom:4 }}>
                    <p style={{ color:"#f1f5f9", fontSize:20, fontWeight:900, margin:0 }}>{w.word}</p>
                    <p style={{ color:C.teal, fontSize:13, margin:0 }}>{w.reading}</p>
                    {w.jlpt && <span style={{ background:"rgba(168,85,247,0.2)", color:C.purpleLight, fontSize:10, fontWeight:700, padding:"1px 7px", borderRadius:99 }}>{w.jlpt}</span>}
                  </div>
                  <p style={{ color:"#94a3b8", fontSize:13, margin:"0 0 8px" }}>{w.meaning}</p>
                  <div style={{ background:"rgba(6,182,212,0.06)", borderRadius:8, padding:"8px 10px" }}>
                    <p style={{ color:"#f1f5f9", fontSize:12, margin:"0 0 2px" }}>{w.example}</p>
                    <p style={{ color:"#64748b", fontSize:11, margin:0, fontStyle:"italic" }}>{w.example_translated}</p>
                  </div>
                </div>
                <button onClick={e=>{e.stopPropagation();speakJapanese(w.word);}} style={{ width:36, height:36, borderRadius:10, background:"rgba(6,182,212,0.1)", border:`1px solid rgba(6,182,212,0.2)`, color:C.teal, fontSize:16, cursor:"pointer", flexShrink:0, marginLeft:8, display:"flex", alignItems:"center", justifyContent:"center" }}>🔊</button>
              </div>
              <p style={{ color:"#475569", fontSize:11, margin:"8px 0 0", textAlign:"right" }}>Tap for full card →</p>
            </div>
          ))}
        </div>
      )}
