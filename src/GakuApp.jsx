  ...
  try {
    ...
    const res = await fetch("/api/claude", {
      ...
      body: JSON.stringify({ model:"claude-sonnet-4-6", max_tokens:5000,
        messages:[{ role:"user", content:`You are a professional UI translator. Translate ALL of these UI strings into ${lang}. Rules: keep emoji exactly as-is, keep * symbols, keep → arrows, sound natural and native. Respond ONLY with a valid JSON object, no markdown, no extra text.
...
async function translateTaskText(task, lang) {
  if (!task || task === "rest" || lang === "English") return task;
  const cacheKey = `task_${lang}_${task}`;
  if (translationCache[cacheKey]) return translationCache[cacheKey];
  try {
    const res = await fetch("/api/claude", {
      method:"POST", headers:{"Content-Type":"application/json"},
      body: JSON.stringify({ model:"claude-sonnet-4-6", max_tokens:150,
        messages:[{ role:"user", content:`Translate this Japanese study app task into ${lang}. Keep time durations (e.g. "14 min") and emoji as-is. Return ONLY the translated text:
"${task}"` }]
      })
    });
    const d = await res.json();
    const translated = d.content?.map(c=>c.text||"").join("").trim() || task;
  ...
  const getHelp = async () => {
    ...
    try {
      ...
      const prompt = isCustom
        ? `You are GAKU, an expert Japanese learning coach and activity designer. You create personalized, engaging Japanese learning experiences.

Student profile:
- Name: ${form.name}
- Level: ${form.jlpt}
- Goal: ${form.goal}
- Skills studying: ${(form.skills||[]).join(", ")}
- Language: ${lang}

The student wants to do something different today: "${diffRequest}"

Your task: Create a COMPLETE, personalized Japanese learning activity based on what they want to do AND provide real resources.

Respond in ${lang} with:
1. A 2-sentence motivating message explaining why their choice is great for Japanese learning
2. ONE complete step-by-step activity (3-5 steps) they can do RIGHT NOW using their request
3. Japanese vocabulary or phrases relevant to their request (5 words/phrases with readings and meanings)
4. EXACTLY 3 real online resources with working URLs

Write the activity and vocabulary first, then end with:
RESOURCES_JSON:[{"name":"Real Name","url":"https://real-working-url.com","desc":"Why it helps"}]

Real URLs to use based on request type:
- ANIME: https://www.crunchyroll.com, https://www.youtube.com/@JapanesePod101, https://learnjapanesewithanime.com
- NEWS: https://www3.nhk.or.jp/news/easy/, https://www.nhk.or.jp/nhkworld/en/learnjapanese/, https://mainichi.jp
- MUSIC: https://www.uta-net.com, https://www.youtube.com/results?search_query=japanese+music, https://open.spotify.com/playlist/37i9dQZF1DXd9rSDyQguIk
- MANGA: https://mangadex.org, https://www.bookwalker.jp, https://comic-walker.com
- DRAMA: https://www.viki.com/explore?country=japan, https://www.netflix.com/browse/genre/10775, https://www.crunchyroll.com
- GAMES: https://www.wanikani.com, https://www.duolingo.com/course/ja/en/Learn-Japanese, https://www.bunpro.jp
- PODCASTS: https://www.nhk.or.jp/podcasts/, https://open.spotify.com/show/4TaHplCk6AMFJcNPJB3jf5, https://nihongonomori.com`
        : `You are GAKU, a friendly Japanese learning coach. 
Student: Level ${form.jlpt}, Skills: ${(form.skills||[]).join(", ")}, Language: ${lang}
Mood: ${mood}, Time: ${time}min, Energy: ${energy}

Create a personalized study plan for right now in ${lang}:
1. One motivating sentence matching their mood/energy
2. A specific activity for exactly ${time} minutes, step-by-step
3. One real resource to use (with URL)

Be warm, specific, and encouraging. Max 150 words.`;
...
function Dashboard({ form, onEdit }) {
  ...
  // Translate schedule tasks into user language
  useEffect(() => {
    if (lang === "English") { setTranslatedSchedule(null); return; }
    const translateAll = async () => {
      const result = {};
      for (const [day, tasks] of Object.entries(schedule)) {
        result[day] = await Promise.all(tasks.map(async t => {
          if (t.rest) return t;
          // For Japanese use hardcoded translations
          if (lang === "Japanese") {
            const taskMap = {
              "Vocabulary review — Anki or saved words (10 min)": "語彙復習 — Ankiまたは保存した単語 (10分)",
              "Speak aloud: summarize today's content in Japanese (5 min)": "音読：今日の内容を日本語で要約 (5分)",
            };
            let translated = taskMap[t.task] || t.task;
            // Translate skill-based tasks
            if (t.task.startsWith("💬 Conversation:")) translated = t.task.replace("💬 Conversation: Role-play or shadowing — CLT core", "💬 会話：ロールプレイまたはシャドーイング — CLTコア");
            if (t.task.startsWith("👂 Listening:")) translated = t.task.replace("👂 Listening: NHK World or JapanesePod101", "👂 リスニング：NHK Worldまたは");
            if (t.task.startsWith("📖 Reading:")) translated = t.task.replace("📖 Reading: Tadoku graded reader or NHK Web Easy", "📖 読解：Tadoku分級読本またはNHK Web Easy");
            if (t.task.startsWith("📝 Grammar:")) translated = t.task.replace("📝 Grammar: Imabi + write 3 example sentences", "📝 文法：Imabi + 例文を3つ書く");
            if (t.task.startsWith("🈳 Kanji:")) translated = t.task.replace("🈳 Kanji: Nihonten AI — 5 new kanji with context", "🈳 漢字：Nihonten AI — 文脈付きで5つの新しい漢字");
            if (t.task.startsWith("🎯 JLPT")) translated = t.task.replace("🎯 JLPT Prep: Japanese Test 4 You — one practice section", "🎯 JLPT対策：Japanese Test 4 You — 練習セクション1つ");
            if (t.task.startsWith("🔊 Pronunciation:")) translated = t.task.replace("🔊 Pronunciation: Anki audio cards — shadow 20 words", "🔊 発音：Ankiオーディオカード — 20単語をシャドーイング");
            return {...t, task: translated};
          }
          const translated = await translateTaskText(t.task, lang);
          return {...t, task: translated};
        }));
      }
      setTranslatedSchedule(result);
    };
    translateAll();
  }, [lang, schedule]);

  const displaySchedule = translatedSchedule || schedule;
      ...
      <div style={{maxWidth:600,margin:"0 auto",padding:"20px 16px"}}>
        ...
        {tab==="schedule"&&(
          <div style={{...S.card}}>
            ...
            {DAY_KEYS.map(dayKey=>{
              const tasks = displaySchedule[dayKey]||[];
              return (
