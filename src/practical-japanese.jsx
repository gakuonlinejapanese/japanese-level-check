...
const QUESTIONS = [
  {
    hiragana: true,
    text: "ともだち に プレゼント を もらった とき、なんと いいますか。",
    options: ["すみません", "ありがとう", "おねがいします", "しつれいします", "No Answer"],
    answer: 1,
    explanation: "「ありがとう」is the natural expression of gratitude when receiving a gift. 「すみません」is used to apologize or get attention, 「おねがいします」to make requests, and 「しつれいします」when leaving or entering a space.",
  },
  {
    cefr:"A1", jlpt:"N5",
    text: "店で水を注文するとき、一番自然なのは？",
    textEn: "You are at a restaurant and want to order water. Which is the most natural phrase?",
    options: ["水。", "水をくれ。", "お水をください。", "水だ。", "No Answer"],
    answer: 2,
    explanation: "「〜をください」is the standard polite request for strangers or service staff. 「くれ」is too blunt; the others are not requests at all.",
  },
  {
    cefr:"A2", jlpt:"N4",
    text: "先生にレポートを送るとき、自然なのは？",
    textEn: "You are submitting a report to your teacher. Which phrase is most natural?",
    options: ["見て。", "見てください。", "見なさい。", "見る？", "No Answer"],
    answer: 1,
    explanation: "「〜てください」is the standard polite request form for teachers or superiors. 「見なさい」is used by authority figures toward subordinates, not the reverse.",
  },
  {
    cefr:"B1", jlpt:"N3",
    text: "明日までに対応できるか聞かれた。職場で一番自然な返答は？",
    textEn: "A colleague asks if you can handle a task by tomorrow. Which reply is most professional?",
    options: ["明日できます。", "明日やる。", "明日対応します。", "明日した。", "No Answer"],
    answer: 2,
    explanation: "「対応します (taiō shimasu)」is standard business Japanese for 'I will handle/deal with it'. It sounds more professional and committed than 「できます」or 「やる」.",
  },
  {
    cefr:"B2", jlpt:"N2",
    text: "上司に仕事をお願いするとき、最も適切なのは？",
    textEn: "You need to ask a senior colleague to do something. Which is the most appropriate request?",
    options: ["これ、やって。", "これ、やってもいい？", "こちらをお願いできますでしょうか。", "これ、する。", "No Answer"],
    answer: 2,
    explanation: "「お願いできますでしょうか」is a layered polite request standard in Japanese workplaces. Though technically a double-polite form, it is widely accepted and expected in formal settings.",
  },
  {
    cefr:"C1", jlpt:"N1",
    text: "書類確認をお願いするビジネスメールで最も適切な表現は？",
    textEn: "You send a business email asking someone to review a document. Which closing is most appropriate?",
    options: ["ご確認ください。", "確認して。", "確認しといて。", "見て。", "No Answer"],
    answer: 0,
    explanation: "「ご確認ください」uses the honorific prefix 「ご〜」with 「ください」, the standard written-form polite request in business emails. The other options are too casual for formal correspondence.",
  },
];

function getResult(score, total) {
  const p = score / total;
  if (p === 1)   return { level:"C1",     msg:"Near-native usage — you sound naturally Japanese!" };
  if (p >= 0.83) return { level:"B2–C1",  msg:"Advanced. Nuanced and professional expression." };
  if (p >= 0.67) return { level:"B1–B2",  msg:"Upper-intermediate. Your Japanese sounds natural." };
  if (p >= 0.5)  return { level:"A2–B1",  msg:"Intermediate. Keep practicing real-world usage." };
  if (p >= 0.33) return { level:"A2",     msg:"Elementary. Focus on polite vs. casual distinctions." };
  if (p > 0)     return { level:"A1",     msg:"Beginner. Learn basic 〜てください patterns first." };
  return               { level:"Pre-A1", msg:"Just starting — real-world usage comes with practice!" };
}

export default function PracticalJapanese() {
      ...
      <div style={S.page}>
        <div style={S.card}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:18 }}>
            <span style={{ color:COLOR, fontWeight:800, fontSize:14 }}>💬 Practical Japanese</span>
          ...
          <p style={S.qText}>{q.text}</p><p style={{color:"#64748b",fontSize:12,marginBottom:8,fontStyle:"italic"}}>💡 If you don't know the answer, click "No Answer".</p>
  ...
  if (phase === "gate") return (
    <div style={S.page}>
      <div style={S.card}>
        ...
        <button onClick={() => { const r=getResult(score,QUESTIONS.length);fetch('https://formspree.io/f/mykvallk',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:name,email:email,score:score+'/'+QUESTIONS.length,cefr:r.level})}).then(()=>setPhase('result')); }} style={{
          ...S.btn,
      ...
      <div style={S.page}>
        <div style={{ ...S.card, maxWidth:580 }}>
          <div style={{ textAlign:"center", marginBottom:26 }}>
            <p style={{ color:"#64748b", fontSize:13, marginBottom:4 }}>{name}'s Results — Practical Japanese</p>
